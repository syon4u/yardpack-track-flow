...
export const useFedExTracking = () => {
  const { toast } = useToast();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ trackingNumber, packageId }: { trackingNumber: string; packageId?: string }) => {
      console.log('Calling FedEx tracking for:', trackingNumber);
      
      const { data, error } = await supabase.functions.invoke('fedex-tracking', {
        body: { trackingNumber, packageId }
      });
    ...
    onSuccess: (data) => {
      if (data.success) {
        toast({
          title: "Tracking Updated",
          description: `Successfully synced FedEx tracking for ${data.tracking_number}`,
        });
        ...
        toast({
          title: "Tracking Failed",
          description: data.error || "Failed to sync FedEx tracking information",
          variant: "destructive",
        });
      }
    },
    onError: (error: any) => {
      ...
      let errorMessage = "Failed to connect to FedEx tracking service";
      if (error.message?.includes('credentials not configured')) {
        errorMessage = "FedEx API not configured. Please configure API keys in System Settings.";
      }
      
      toast({
        title: "FedEx Tracking Error",
        description: errorMessage,
        variant: "destructive",
      });
    },
...
export const useUPSTracking = () => {
  const { toast } = useToast();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ trackingNumber, packageId }: { trackingNumber: string; packageId?: string }) => {
      console.log('Calling UPS tracking for:', trackingNumber);
      
      const { data, error } = await supabase.functions.invoke('ups-tracking', {
        body: { trackingNumber, packageId }
      });
    ...
    onSuccess: (data) => {
      if (data.success) {
        toast({
          title: "Tracking Updated",
          description: `Successfully synced UPS tracking for ${data.tracking_number}`,
        });
        ...
        toast({
          title: "Tracking Failed",
          description: data.error || "Failed to sync UPS tracking information",
          variant: "destructive",
        });
      }
    },
    onError: (error: any) => {
      ...
      let errorMessage = "Failed to connect to UPS tracking service";
      if (error.message?.includes('credentials not configured')) {
        errorMessage = "UPS API not configured. Please configure API keys in System Settings.";
      }
      
      toast({
        title: "UPS Tracking Error",
        description: errorMessage,
        variant: "destructive",
      });
    },
